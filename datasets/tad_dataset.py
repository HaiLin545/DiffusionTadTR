...
class TADDataset(torch.utils.data.Dataset):
    ...
    def _get_feature_data(self, index):
        ...
        local_ft_path = (
            osp.join(local_ft_dir, feature_info["fn_templ"] % src_vid_name)
            if local_ft_dir
            else None
        )
...
def build(dataset, subset, args, mode):
    """build TADDataset"""
    subset_mapping, feature_info, ann_file, ft_info_file = get_dataset_info(
        dataset, args.feature
    )
    transforms = None
    if args.input_type == "image":
        ...
        transforms = None
    return TADDataset(
        subset_mapping[subset],
        mode,
        feature_info,
        ann_file,
        ft_info_file,
        transforms,
        online_slice=args.online_slice,
        slice_len=args.slice_len,
        slice_overlap=args.slice_overlap
        if mode == "train"
        else args.test_slice_overlap,
        binary=args.binary,
