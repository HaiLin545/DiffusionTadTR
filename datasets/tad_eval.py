...
class TADEvaluator(object):
    def __init__(
        ...
        unique_video_list = [
            x for x in database if database[x]["subset"] in subset_mapping[subset]
        ]
        ...
        for vid in unique_video_list:
            if vid in self.ignored_videos:
                continue
            this_gts = [
                x for x in database[vid]["annotations"] if x["label"] != "Ambiguous"
            ]
            all_gt += [
                [vid, classes.index(x["label"]), x["segment"][0], x["segment"][1]]
                for x in this_gts
            ]
        ...
        logging.info(
            "{} ground truth instances from {} videos".format(
                len(all_gt), len(self.video_ids)
            )
        )
        ...
        for k, v in pred_numpy.items():
            ...
            for nms_mode in self.nms_mode:
                ...
                if assign_cls_labels:
                    raise NotImplementedError
                self.all_pred[nms_mode] += [
                    [video_id, k] + det for det in dets.tolist()
                ]
    ...
    def nms_whole_dataset(self):
        ...
        for vid in video_ids:
            this_dets = self.all_pred["nms"][self.all_pred["nms"]["video-id"] == vid][
                ["t-start", "t-end", "score", "cls"]
            ].values

            this_dets = apply_nms(this_dets)[:200, ...]
            this_dets = [[vid] + x.tolist() for x in this_dets]
            all_pred += this_dets
        self.all_pred["nms"] = pd.DataFrame(
            all_pred, columns=["video-id", "t-start", "t-end", "score", "cls"]
        )
        ...
        for vid in video_ids:
            ...
            if len(slice_ids) > 1:
                ...
                for i in range(0, len(slice_sorted) - 1):
                    ...
                    overlap_region_time = [
                        time_base + overlap_region_time[iii] / feature_fps
                        for iii in range(2)
                    ]
                ...
                for overlap_region_time in overlap_region_time_list:
                    inters = np.minimum(
                        this_dets["t-end"], overlap_region_time[1]
                    ) - np.maximum(this_dets["t-start"], overlap_region_time[0])
                    # we only perform NMS to the overlapped regions
                    mask = inters > 0
                    overlap_dets = this_dets[mask]
                    overlap_dets_arr = overlap_dets[
                        ["t-start", "t-end", "score", "cls"]
                    ].values
                    if len(overlap_dets) > 0:
                        kept_dets_arr = apply_nms(
                            np.concatenate(
                                (
                                    overlap_dets_arr,
                                    np.arange(len(overlap_dets_arr))[:, None],
                                ),
                                axis=1,
                            )
                        )
                        processed_dets.append(
                            overlap_dets.iloc[kept_dets_arr[:, -1].astype("int64")]
                        )
    ...
    def accumulate(self, test_slice_overlap=0):
        ...
        for nms_mode in self.nms_mode:
            self.all_pred[nms_mode] = pd.DataFrame(
                self.all_pred[nms_mode],
        ...
        for nms_mode in self.nms_mode:
            ...
            self.pred_by_cls[nms_mode] = [
                self.all_pred[nms_mode][self.all_pred[nms_mode].cls == cls]
    ...
    def synchronize_between_processes(self):
        ...
        print(len(self.all_pred[mode]), len({x[0] for x in self.all_pred[mode]}))
    ...
    def summarize(self):
        ...
        header = (
            " ".join("%.2f" % self.iou_range[i] for i in display_iou_thr_inds) + " avg"
        )  # 0 5 9
        lines = []
        for nms_mode in self.nms_mode:
            per_iou_ap = self.compute_map(nms_mode)
            line = (
                " ".join(["%.2f" % (100 * per_iou_ap[i]) for i in display_iou_thr_inds])
                + " %.2f" % (100 * per_iou_ap.mean())
                + " {} epoch{}".format(nms_mode, self.epoch)
            )
            lines.append(line)
        msg = header
        for l in lines:
    ...
    def compute_map(self, nms_mode):
        ...
        with concurrent.futures.ProcessPoolExecutor(min(self.num_workers, 8)) as p:
            ...
            for cls in range(len(pred_by_cls)):
                ...
                futures.append(
                    p.submit(eval_ap, iou_range, cls, gt_by_cls[cls], pred_by_cls[cls])
                )
                ...
                det_list.append(
                    {
                        ...
                        "label": self.classes[int(row["cls"])],
    ...
    def dump_detection(self, save_path=None):
        for nms_mode in self.nms_mode:
            logging.info(
                "dump detection result in JSON format to {}".format(
                    save_path.format(nms_mode)
                )
            )
