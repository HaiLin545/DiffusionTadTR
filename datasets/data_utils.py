...
def get_valid_anno(
    ...
    for inst in gt_instances:
        ...
        if end_getter(inst) <= start or start_getter(inst) >= end:
            ...
            integrity = (
                (new_end - new_start) * 1.0 / (end_getter(inst) - start_getter(inst))
            )
...
def get_dataset_dict(
    ...
    for video_name in video_list:
        ...
        annotations = [
            x for x in anno_data[video_name]["annotations"] if x["label"] != "Ambiguous"
        ]
        ...
        if online_slice:
            ...
            if feature_length <= slice_len:
                ...
                num_complete_slices = int(
                    math.floor(
                        (feature_length / slice_len - 1) / (1 - slice_overlap) + 1
                    )
                )
                slices = [
                    [int(i * stride), int(i * stride) + slice_len]
            ...
            for slice in slices:
                time_slices = [
                    slice[0] / video_info["feature_fps"],
                    slice[1] / video_info["feature_fps"],
                ]
                ...
                if not ignore_empty or len(valid_annotations) >= 1:
                    ...
                    new_vid_info = {
                        ...
                        "src_vid_name": video_name,
                        "feature_fps": feature_fps,
                        "feature_length": slice_len,
                        "subset": subset,
                        "feature_second": feature_second,
                        "time_offset": time_slices[0],
                    }
        ...
        # for ActivityNet and hacs, use the full-length videos as samples
            ...
            if not ignore_empty or len(annotations) >= 1:
                ...
                valid_annotations = [
                    x for x in annotations if x["segment"][1] - x["segment"][0] > 0.02
                ]
...
def get_dataset_info(dataset, feature):
    ...
    if dataset == "thumos14":
        ...
        if feature == "i3d2s":
            feature_info = {
                "local_path": path_info["thumos14"][feature]["local_path"],
                "format": "torch",
                "fn_templ": "%s",
            }
