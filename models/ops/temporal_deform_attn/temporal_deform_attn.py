...
class DeformAttn(nn.Module):
    ...
    def forward(
        ...
        # deform attention in the l-th (l >= 2) decoder layer when segment refinement is enabled
            ...
            raise ValueError(
                "Last dim of reference_points must be 1 or 2, but get {} instead.".format(
        ...
        if cfg.dfm_att_backend == "pytorch" or cfg.disable_cuda:
            ...
            sampling_locations = torch.cat(
                (sampling_locations, torch.ones_like(sampling_locations) * 0.5), dim=-1
            )
            input_spatial_shapes = torch.stack(
                (torch.ones_like(input_temporal_lens), input_temporal_lens), dim=-1
            )
