...
def train_one_epoch(
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        targets = [
            {
                k: v.to(device) if k in ["segments", "labels"] else v
                for k, v in t.items()
            }
            for t in targets
        ]
...
def test(
    ...
    iou_range = (
        [0.3, 0.4, 0.5, 0.6, 0.7]
        if cfg.dataset_name == "thumos14"
        else [num / 100 for num in range(50, 100, 5)]
    )
    # logging.info('iou range {}'.format(iou_range))

    # action_evaluator = None
    action_evaluator = TADEvaluator(
        cfg.dataset_name,
        subset,
        base_ds,
        nms_mode=["raw"],
